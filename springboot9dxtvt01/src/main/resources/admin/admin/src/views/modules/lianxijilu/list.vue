		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"1px solid #bababa","padding":"20px","margin":"0px","overflow":"hidden","borderRadius":"8px","background":"#fff","fontSize":"inherit"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#333","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('lianxijilu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('lianxijilu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('lianxijilu','错题分析')" type="success" @click="chartDialog1()">
			...
			<div :style='{"border":"1px solid #bababa","width":"100%","padding":"0","margin":"30px 0 0","borderRadius":"8px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="错题次数">
						<template slot-scope="scope">
							{{scope.row.cuoticishu}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="cuotibaogao" label="错题报告">
						<template slot-scope="scope">
							<el-button v-if="scope.row.cuotibaogao" type="text" size="small" @click="download(scope.row.cuotibaogao)">下载</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('lianxijilu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('lianxijilu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('lianxijilu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
//统计接口
	...
	lianxijilustatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'lianxijilu/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
