...
<div class="content" :style='{"minHeight":"100vh","padding":"20px 30px 20px","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20231021/28c42ea3e00540059cfef5c493c3e25b.png) no-repeat center top / cover","display":"flex","fontSize":"14px","height":"auto"}'>
	...
	<!-- statis -->
	<div :style='{"border":"1px solid #bababa","padding":"20px 10px 10px 20px","margin":"0 0 30px","alignItems":"center","color":"#ababab","display":"flex","justifyContent":"flex-start","borderRadius":"8px","flexWrap":"wrap","background":"#fff","width":"100%","fontSize":"14px","order":"0"}'>
		<div :style='{"padding":"20px 10px","margin":"0 2% 12px 0","borderColor":"#e7e8f8","borderRadius":"8px","alignItems":"center","textAlign":"center","background":"#f4f4f4","borderWidth":"0px","display":"flex","width":"18%","borderStyle":"solid","justifyContent":"flex-start"}' v-if="isAuth('lianxijilu','首页总数')">
			<div :style='{"alignItems":"center","borderRadius":"100%","background":"#6bccc9","display":"flex","width":"50px","justifyContent":"center","height":"50px"}'>
				<span class="icon iconfont icon-touxiang09" :style='{"color":"#fff","fontSize":"32px"}'></span>
			</div>
			<div :style='{"width":"auto","margin":"0 0 0 20px","textAlign":"left","flexDirection":"column-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"500","height":"24px"}'>{{lianxijiluCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#737373","height":"24px"}'>练习记录总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 1 -->
	<div class="type1" :style='{"alignContent":"flex-start","padding":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto","order":"10"}'>
		<div id="lianxijiluChart1" class="echarts1" v-if="isAuth('lianxijilu','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getlianxijiluCount();
		this.lianxijiluChat1();
	},
	methods:{
		// 词云
		...
		getlianxijiluCount() {
			this.$http({
				url: `lianxijilu/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.lianxijiluCount = data.data
				}
			})
		},
		lianxijiluChat1() {
        ...
        var lianxijiluChart1 = echarts.init(document.getElementById("lianxijiluChart1"),'macarons');
        this.$http({
            url: `lianxijilu/valueMul/xingming?yColumnNameMul=daticishu,cuoticishu`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                lianxijiluChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    lianxijiluChart1.resize();
                };
            }
        });
      })
